...
const Column = ({ title, value }: {title: string, value: string}) => {
  ...
  for (const item in tasks) {
    taskArray = taskArray.concat(tasks[item]);
  }

  const itemArray = taskArray.filter((item: Task) => item.status === value);
  ...
  return(
    <div className="flex flex-col snap-center gap-4">
      ...
      {taskArray.some((item: Task) => value.includes(item.status)) ? (
        <h1>{`${title} (${itemArray.length})`}</h1>
      ) : null}
      {itemArray.map((item: Task) => {
        return (
          <div key={item.id}>
            <Card itemCard={item} />
          </div>
